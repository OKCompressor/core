...
def banded_ngram_pipeline(config, infile, ccnlp_out):
    ...
    Aggregates n-grams for each band, then does a single replacement+prune for memory efficiency.
    """
    from steps.step_03_replace_ngrams import step_replace_ngrams
    from modules.ngram_pos.dict_pruner import frequency_prune_commons, remap_subidxs_with_mapping
    ...
    for pass_idx, band in enumerate(config['ngram_passes']):
        ...
        last_band = band  # Track for replacement
    ...
    # Frequency-aware pruning of commons dicts
    for dict_file in all_commons_dicts:
        ...
        mapping = frequency_prune_commons(
...
def step_ngram_aggregate(config, infile, ccnlp_out, n_max, n_min, min_freq, start_code):
    ...
    script = os.path.join(modules_dir, "ngram_pos", "aggregate.py")  # ← CORRECT legacy dash, not underscore
    ...
    args = {
        ...
        "n_min": n_min,
        "min_freq": min_freq,
        # If you want to support start_code in future, add CLI support to aggregate.py!
        "output": output_npz,
        "sqlite_db": output_db
    }
